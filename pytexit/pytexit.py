...
#                child = [self.group(read_tree(tc)) for tc in t['up']]
    ...
    def convert_symbols(self,expr):
        ...
        # lambda_i might nevertheless been used
        elif m in ['lambda']: 
            m = r'\lambda'
    ...
    # fixed 'A - (B + C)', formerly parentheses were missing in the result
    def visit_BinOp(self, n):
        if self.prec(n.op) > self.prec(n.left):
            left = self.parenthesis(self.visit(n.left))
        else:
            left = self.visit(n.left)
        # the the second condition was added for the fix
        if (self.prec(n.op) > self.prec(n.right)) or (isinstance(n.op, ast.Sub) and (self.prec(n.op) == self.prec(n.right))):
    ...
    def visit_Mult(self, n):
        return r'\cdot ' # reduced space from: r'\;'
